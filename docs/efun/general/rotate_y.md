...
### NAME
    ...
    rotate_y() - rotate a transformation matrix about the Y axis

### SYNOPSIS

    float *rotate_y(float *matrix, float degrees)

### DESCRIPTION

    Post-multiplies the 4x4 transformation `matrix` (a 16-element flat
    array of floats, as produced by id_matrix()) by a rotation of
    `degrees` about the Y axis, computing `matrix` = `matrix` * rotation.
    ...
    as the return value. rotate_y() mutates its first argument; the return
    value is the very array you passed in, not a copy.

### SEE ALSO

    rotate_x(3), rotate_z(3), id_matrix(3)
