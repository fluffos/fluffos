...
LPC is the language in which FluffOS (and other LPmud driver) objects are
written. LPC stands for Lars Pensjö C. As one might surmise from the name,
...
strcpy(ack, foo);
ack = (char *)realloc(ack, strlen(ack) + strlen(bar) + 1);
strcat(ack, bar);
```
...
`sscanf()` does not work in the same way as in C. Arguments to `sscanf()` need
not be pointers (since LPC does not have the explicit pointer data type). Also,
