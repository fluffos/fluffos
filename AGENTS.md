...
* The array is a normal DMALLOC block (`TAG_LOCAL_NAMES`, `base/internal/debugmalloc.h`), freed in `deallocate_program()` and in `epilog()`'s superseded-function-entry cleanup, and marked in `checkmemory.cc`'s `TAG_PROGRAM` case the same way `file_info`/`TAG_LINENUMBERS` already is. A missing mark there would flood any Debug-build LPC suite run with orphan-block warnings the instant `debugger port` is set -- treat future edits to this table's shape as touching a DMALLOC-tracked block, not a plain heap array (AGENTS §3's off-graph-reference rules apply).
