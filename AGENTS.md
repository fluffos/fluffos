...
* **libwebsockets** ws-over-h2 (RFC 8441) fixes, MERGED upstream (warmcat/libwebsockets#3634) and kept vendored until the next lws version bump: (1) `lib/roles/ws/ops-ws.c` `rops_write_role_protocol_ws` fires `LWS_EXT_CB_PACKET_TX_PRESEND` before delegating an encapsulated frame to the h2 role -- without it permessage-deflate compressed the payload but never set RSV1, so peers rendered raw deflate bytes ("garbled output"); (2) `lib/roles/h2/http2.c` `lws_h2_frame_write` PARKS a whole carries-ws DATA frame on the stream's `buflist_out` when h2 tx credit is short (sending past the peer's window is a connection-fatal FLOW_CONTROL_ERROR), `lws_h2_ws_drain_parked_tx` (new, called from `rops_perform_user_POLLOUT_h2`'s buffered-out branch in `ops-h2.c`) sends parked frames oldest-first as WINDOW_UPDATE restores credit, and `lws_send_pipe_choked` (`plat/unix/unix-sockets.c`, `plat/windows/windows-sockets.c`) reports a stream with parked frames as choked so user write loops back off; (3) `lib/roles/ws/ops-ws.c` `rops_callback_on_writable_ws` returns cleanly when the h2 parent is already unlinked mid-teardown instead of dereferencing NULL (release builds compile out the `assert(enc)`); (4) `lib/roles/ws/ops-ws.c` the h2-encapsulation branch of `rops_write_role_protocol_ws` returns **orig_len** on success like the h1 path -- it used to return the post-extension frame size, which permessage-deflate routinely SHRINKS below the caller's payload length, making well-behaved callers (fluffos's own `m < numbytes` check included) conclude the write failed short and kill the connection (the "Firefox connects then immediately disconnects" bug -- Chromium was immune only because it uses h1 ws); (5) `lib/roles/h2/ops-h2.c` `rops_perform_user_POLLOUT_h2` services `tx_draining_ext` for carries-ws children (`lws_write(w, NULL, 0, LWS_WRITE_CONTINUATION)`, mirroring `rops_handle_POLLOUT_ws` priority 5) -- the mux servicing loop is the ONLY POLLOUT path an encapsulated child gets, and while the extension is draining `lws_send_pipe_choked()` reports choked, so without this any message whose compressed output overflows pmd's chunk buffer wedges the stream permanently. The merged upstream series additionally carries fixes for client-side RFC 8441 extended CONNECT and a ws-role `tx_credit` rops -- both deliberately NOT vendored (the client feature doesn't exist in this v4.5.x, which hard-rejects client ws-over-h2, and the driver is ws-server-only; the tx_credit rops has no in-tree consumer). ONE DELIBERATE DIVERGENCE from merged upstream: our parking/drain/re-arm sites gate on the stream's OWN `buflist_out`, while upstream merged the broader `lws_has_buffered_out()` gating, which spuriously closes a carries-ws child serviced while the NETWORK wsi holds a partial socket write (real-network backpressure only; invisible on localhost, so the in-tree api-test passes either way) -- a follow-up patch fixing this upstream is pending. When bumping lws past v4.5.x, take the upstream series and verify the buflist_out gating follow-up has landed (or re-apply it) before dropping the local patches.
...
* **The test suites do not exercise a websocket client connection** -- the vhost-adoption regression above passed the full GTest + LPC suites. After touching lws or `src/net/`, run a real end-to-end smoke test: boot `driver etc/config.test`, complete a 101 upgrade handshake on the plain and TLS ws ports with the `ascii`/`telnet`/`binary` subprotocols, and confirm the first data frame arrives — AND that a burst larger than one 2048-byte write window arrives completely (e.g. `tuidemo charts` through `src/www/index.html`) — AND repeat the burst on an h2 (RFC 8441 extended CONNECT) connection **with `permessage-deflate` offered**, verifying the inflated content, plus one message whose COMPRESSED size exceeds ~1KB (incompressible data) to exercise the extension tx-drain path. Firefox does h2+pmd, Chromium does h1: a matrix that skips either combination has shipped "works in Chrome, dies in Firefox" bugs twice (the RSV1 garbling and the short-return disconnect).
...
* **Hit-count conditions**: DAP `hitCondition` per source breakpoint -- an optional comparator (`> >= < <= == = !=` or `%`) followed by an integer, bare number meaning `>=N` -- parsed and validated once in `set_breakpoints_request()` (`breakpoints.cc`); a syntactically bad condition marks the breakpoint permanently `verified: false` with a `message`, and never registers an address (so it can never be hit at all, rather than silently behaving as unconditional). `SrcBp::hit_count` accumulates across the breakpoint's lifetime; `g_session.bp_by_id` (bp id -> `SrcBp*`, rebuilt in `rebind_all()` since `set_breakpoints_request()` always replaces a whole canonical path's vector wholesale) is what lets the instruction hook's address match (`bp_addrs`) reach the right hit-count state without a linear scan. `breakpoint_hit_should_stop()` re-parses the condition on every hit rather than caching it -- deliberately: this only runs after an address already matched, so it's off the per-instruction hot path, and one parser stays the single source of truth for both validation and evaluation.
* **Breakpoints inside `#include`d header files resolve correctly.** This was never a `breakpoints.cc` bug -- `file_ids_for()`/`line_run_starts()` already walk every file in a program's `file_info` table generically, and `.h` paths compare correctly (`strip_ext()` only touches `.lpc`/`.c`). The real cause was upstream in the compiler (fixed; see `DESIGN.md`'s §15 phasing note for the full root-cause writeup): a `return <constant>;`/`return;` statement can be built entirely from `new_node_no_line()` nodes, so `i_generate_node()`'s `if (expr->line && ...)` guard (`compiler/internal/icode.cc`) never called `switch_to_line()` for it, silently attributing its bytecode to whichever line was already active -- `abs_line 0` if it was the first thing generated in the compile unit, which an `#include`d header's leading function always is. Fixed by having `rule_return_void()`/`rule_return_expr()` (`grammar_rules_loops.cc`) stamp a real line onto every explicit, user-written return statement, plus a safety net in `rule_func()` (`grammar_rules.cc`) that stamps every function's own declaration line onto its `NODE_FUNCTION` node (captured early in `rule_func_type()`, before parser lookahead can drift into a different file) so even a function with an implicit return or empty body still gets an accurate line. Regression-tested by `BreakpointInsideIncludedHeaderFileResolves` and `BreakpointOnImplicitReturnHeaderFunctionResolves` in `test_debugger.cc`.

### Local variable names
* The compiler discards local/parameter NAMES after each function body compiles (`free_all_local_names()` in `compiler/internal/compiler.cc`) -- only the `num_arg`/`num_local` counts normally survive in `function_t`. Whenever `debugger port` is set (no separate rc option), `rule_func()` (`compiler/internal/grammar_rules.cc`) captures them into a `function_t::local_names` array -- one `store_prog_string()` string-pool index per runtime slot, `-1` for an uncaptured slot -- *before* that free fires; consumed by `inspect.cc`'s `local_name_for()`. Names are per-program and baked in at compile time: an object compiled before `debugger port` was configured still shows `argN`/`localN` for its whole lifetime, even if a debugger later attaches.
* **Known caveats**: a `for()`/`switch()`-scoped local's slot can be permanently unrecoverable if its block already closed by the time the function finished compiling (`pop_n_locals()` drops it from the capture source without releasing its runtime slot, so a later local can land at a higher slot number, leaving a real gap) -- falls back to the index name for just that slot; `LocalNamesLeaveGapForScopedOutLoopVariable` (`test_debugger.cc`) pins this shape. Anonymous/lambda functions are never instrumented -- `rule_func()` is the only capture site -- so `FRAME_FUNP` frames always show index names.
* The array is a normal DMALLOC block (`TAG_LOCAL_NAMES`, `base/internal/debugmalloc.h`), freed in THREE places: `deallocate_program()` (normal teardown), `epilog()`'s superseded-function-entry cleanup (a successful compile with duplicate/dead `function_t` slots), and `clean_parser()` (an ABORTED compile -- a parse error, or an inherit/include denied by a master apply -- discards every entry in `mem_block[A_FUNCTIONS]`, not just the superseded tail; missing this one was a real leak, only visible with `debugger port` forced on for a full testsuite run). Marked in `checkmemory.cc`'s `TAG_PROGRAM` case the same way `file_info`/`TAG_LINENUMBERS` already is -- a missing mark there would flood any Debug-build LPC suite run with orphan-block warnings the instant `debugger port` is set. Treat future edits to this table's shape as touching a DMALLOC-tracked block with three teardown paths, not a plain heap array (AGENTS §3's off-graph-reference rules apply). **A new `TAG_*` constant's "+n" suffix must be globally unique across every category** (`debugmalloc.h`'s own comment says so) -- `checkmemory.cc`'s `check_string_stats()` reads `blocks[TAG_SHARED_STRING & 0xff]`/`blocks[TAG_MALLOC_STRING & 0xff]`, which folds away the category (high byte), so a same-suffix collision with an unrelated category (this happened: `TAG_PERMANENT + 41` vs `TAG_MALLOC_STRING = TAG_DATA + 41`) silently corrupts live string-count accounting instead of erroring.

### Editing values (`setVariable`)
* Resolves the DAP `setVariable` target by re-walking the same container cases `build_variables()` (`inspect.cc`) renders, matching the exact `name` string a prior `variables` response gave that child -- frame args/locals (real name if captured, else `argN`/`localN`), object globals, array/class elements by their `"[i]"`/`"member [i]"` text, mapping values by their (possibly-truncated) key preview text. A mapping preview collision (two keys rendering identically) is rejected as ambiguous rather than risking a write to the wrong entry.
* New values are parsed by a deliberately tiny literal grammar -- integer, float, or a double-quoted string (`\" \\ \n \t \r` escapes) -- never arrays/mappings/objects/function literals, which have no textual form here and always fail cleanly (that's what phase 3's `evaluate` is for). Assignment goes through `assign_svalue()` for correct ref-count handling. Only reachable while stopped (same guard as `variables`/`stackTrace`).
...
* **`master::valid_debugger(remote_ip)`** (`docs/apply/master/valid_debugger.md`): an optional mudlib-side veto consulted in `handle_request()`'s `attach` branch (`debug_server.cc`), after the password check succeeds and before the session is marked attached. Permissive when undefined -- `if (mret && !MASTER_APPROVED(mret))`, the `valid_object`-style guard, not the bare `valid_socket`-style check -- so a mudlib that configured `debugger port`/`debugger password` before this apply existed is never silently locked out by a driver upgrade. `remote_ip` is captured once per connection at `LWS_CALLBACK_ESTABLISHED` (`transport_lws.cc`) via `lws_get_peer_simple()` (no reverse DNS) into `Session::client_addr` -- there was previously no per-connection address anywhere in the transport. Gated on `!s.attached` so a redundant second `attach` on an already-attached session (reachable from inside the stop loop, since DAP requests are still serviced while stopped) never double-invokes the apply.

### Testing
* `src/tests/test_debugger.cc` (GTest): line-table resolution (exact match / snap-to-next-line / pending-until-loaded), program-free invalidation, the detached no-op contract for the hook API, local-name capture (captured/not-captured/scoped-out-gap), `setVariable` (object global, array element, mapping value, and rejection of unsupported literal syntax), and the `valid_debugger` veto (`AttachSucceedsWhenValidDebuggerApproves`/`AttachDeniedWhenValidDebuggerRejects`, via `dbg::dispatch_message()` -- the same entry point the real transport calls, with no live `lws*` needed since `transport_send()`/`transport_kill_client()` both already guard on `s.client`) -- the frame-scoped arg/local `setVariable` path needs a live `csp`/`fp`, which only a real stopped VM has, so it's covered by dap-smoke.js instead, not here. Loading a throwaway fixture object needs the same `save_context`/`try`/`restore_context`/`pop_context` guard as `test_lpc.cc`'s `TestCompileDumpProgWorks` -- `load_object_from_source()` can run `create()`/`__INIT`/master applies, and an `error()` with no established recovery point hits the driver's `fatal()` fallback and aborts the whole test binary. `LoadFixture`'s `callcreate` parameter defaults to `false` (skips `create()`/`__INIT`, so a global with an initializer like `int x = 10;` stays at its uninitialized `T_NUMBER(0)`) -- pass `true` when a test needs the initializer to actually run (e.g. to get a real `array_t*`/`mapping_t*` out of a global for a `setVariable` container test). A freshly-loaded object's ref count reflects it being alive in `obj_list`/otable, not a reference the caller owns -- do not `free_object()` it directly (`dealloc_object()` fatals on a non-destructed object outside one specific sentinel state); either leave it loaded (the GTest binary is short-lived) or `destruct_object()` it first.
* `tools/dap-smoke.js` is the full-fidelity regression check -- it drives a real session over a real socket against the real driver and is the only test that exercises the transport, the stop-the-world pause, and the auto-resume safety net together; it's also the only place frame-scoped `setVariable` and real (non-fixture) local-name capture get exercised, against `/single/master.lpc`'s `connect()`. Its `setVariable` check restores the local it probes (`err`) back to falsy `0` before continuing -- `connect()`'s `this_object()` is the MASTER object, and leaving `err` truthy would hit `destruct(this_object())` and take the rest of the boot down with it. Run it against both a `RelWithDebInfo` and a Debug+ASan/UBSan driver after touching anything under `src/debugger/`.
