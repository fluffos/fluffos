...
* **Known gap: a breakpoint inside an `#include`d header file never verifies.** Not a `breakpoints.cc` bug -- `file_ids_for()`/`line_run_starts()` already walk every file in a program's `file_info` table generically, and `.h` paths compare correctly (`strip_ext()` only touches `.lpc`/`.c`). The real cause is upstream in the compiler: the first function generated right after an `#include` boundary gets its bytecode attributed to `abs_line 0` (`i_generate_node()`'s `if (expr->line && ...)` guard in `compiler/internal/icode.cc` skips `switch_to_line()` when a node's line is 0), which resolves to a bogus `(includer, line 0)` pair instead of the header's real line. See `DESIGN.md`'s §15 phasing note for the full repro; pinned (not fixed) by `BreakpointInsideIncludedHeaderFileDoesNotYetResolve` in `test_debugger.cc`.
