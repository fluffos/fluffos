...
| `vscode/` | VS Code extension: declarative highlighting (generated TextMate grammar), structural diagnostics as you type, Format Document / format-on-save (`lpc.format.printWidth` default 100, `lpc.format.indentSize` default 2), real `lpcc` compiler errors on save. See `vscode/README.md`. |
| `test.mjs` | `node tools/lpc-syntax/test.mjs` — tokenizer/highlighter/formatter/linter/generated-VS-Code-asset assertions (dependency-free). |
