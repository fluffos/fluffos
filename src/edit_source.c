...
create_option_defines() {
	...
	for (p = defns[i]; p; p = p->next) 
		...
		if (!strcmp(p->name, "MCCP_SUPPORT") && !have_zlib_h)
		    continue;
		count++;
...
#define NUM_FILES     5
	...
	if (!files[i]) {
	    fprintf(stderr, "edit_source: unable to open %s\n", outfiles[i]);
	    exit(-1);
	}
	fprintf(files[i], 
		"/*\n\tThis file is automatically generated by edit_source.\n");
	fprintf(files[i],
...
static void handle_configure() {
    ...
    /* MCCP_SUPPORT stuff */
    if (lookup_define("MCCP_SUPPORT")) {
	printf("Checking for include file <zlib.h> ...");
	if (check_prog("HAVE_ZLIB_H", "#include <zlib.h>", "", 0)) {
	    have_zlib_h = 1;
	    printf(" exists\n");
	} else printf(" does not exist - MCCP_SUPPORT disabled.\n");
    }
	...
	if (lookup_define("USE_POSTGRESQL")) {
	    if (!(check_include("INCL_LOCAL_PGSQL_LIBPQ_FE_H", "/usr/local/include/pgsql/libpq-fe.h")
		|| check_include("INCL_PGSQL_LIBPQ_FE_H", "/usr/include/pgsql/libpq-fe.h"))) {
		fprintf(stderr, "Cannot find libpq-fe.h, compilation is going to fail miserably.\n");
	    }
	}
    }
    ...
    /* MCCP stuff */
    if (lookup_define("MCCP_SUPPORT") && have_zlib_h) {
	if (!check_library("-lz")) {
	    fprintf(stderr, "Cannot find libz.a, compilation may fail at the end.\n");
	}
    }
    ...
    if (lookup_define("PACKAGE_DB") && lookup_define("USE_POSTGRESQL")) {
	if (!(check_library("-lpq") ||
	      check_library("-L/usr/local/lib -lpq") ||
	      check_library("-L/usr/local/lib/pgsql -lpq"))) {
