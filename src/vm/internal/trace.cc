    ...
    for (p = &control_stack[0]; p <= csp ; p++) {
        ...
        switch (p[0].framekind & FRAME_MASK) {
            ...
            case FRAME_FUNP: {
                ...
                dump_trace_line(tmpbuf.buffer, trace_prog->filename, trace_obj->obname, get_line_number(trace_pc, trace_prog));
            ...
            case FRAME_FAKE:
                dump_trace_line("<fake>", trace_prog->filename, trace_obj->obname, get_line_number(trace_pc, trace_prog));
                num_arg = -1;
                break;
            case FRAME_CATCH:
                dump_trace_line("<catch>", trace_prog->filename, trace_obj->obname, get_line_number(trace_pc, trace_prog));
                num_arg = -1;
                break;
            default:
#ifdef DEBUG
