...
object_t *clone_object(const std::string, int);
object_t *environment(svalue_t *);
object_t *first_inventory(svalue_t *);
object_t *object_present(svalue_t *, object_t *);
object_t *find_object(const std::string);
object_t *find_object2(const std::string);
...
void shout_string(const std::string);

[[noreturn]] void error_needs_free(char *);
[[noreturn]] void throw_error(void);
[[noreturn]] void error_handler(std::string);
