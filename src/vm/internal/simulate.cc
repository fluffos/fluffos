...
static object_t *load_virtual_object(const char *name, int clone) {
  ...
  ObjectTable::remove(new_ob);
  if (new_ob->obname) {
    FREE((char *)new_ob->obname);
  }
  SETOBNAME(new_ob, new_name);
  ObjectTable::insert(new_ob);
...
object_t *load_object(const char *lname, int callcreate) {
  ...
  if (inherit_file) {
    ...
    if ((inh_obj = ObjectTable::find(inhbuf) )) {
#ifdef DEBUG
  ...
  obj_list = ob;
  ObjectTable::insert(ob); /* add name to fast object lookup table */
  save_command_giver(command_giver);
  push_object(ob);
...
object_t *clone_object(const char *str1, int num_arg) {
  ...
  ObjectTable::insert(new_ob); /* Add name to fast object lookup table */
