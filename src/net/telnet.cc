...
static inline void on_telnet_data(const char* buffer, unsigned long size,
    interactive_t *ip) {
...
static inline void on_telnet_send(const char* buffer, unsigned long size,
    interactive_t *ip) {
...
static inline void on_telnet_will(unsigned char cmd, interactive_t *ip) {
  ...
  case TELNET_TELOPT_NEW_ENVIRON: {
    ...
    telnet_subnegotiation(ip->telnet,
    TELNET_TELOPT_NEW_ENVIRON, buf, sizeof(buf));
    break;
  }
...
static inline void on_telnet_subnegotiation(unsigned char cmd, const char* buf,
    unsigned long size, interactive_t *ip) {
  switch (cmd) {
  ...
  case TELNET_TELOPT_LINEMODE: {
    ...
    switch (action) {
      ...
      if (!(buf[1] & MODE_ACK)) {
        ...
        const unsigned char sb_ack[] = {
            LM_MODE, MODE_EDIT | MODE_TRAPSIG | MODE_ACK
        };
        telnet_subnegotiation(
            ip->telnet, TELNET_TELOPT_LINEMODE,
            (const char *)sb_ack, sizeof(sb_ack));
      }
      break;
      case LM_SLC: {
      ...
      case DO: {
        ...
        telnet_subnegotiation(ip->telnet,
            TELNET_TELOPT_LINEMODE, (const char *)sb_wont, sizeof(sb_wont));
        break;
      }
      case WILL: {
        const unsigned char sb_dont[] = {DONT, (unsigned char)buf[1]};
        telnet_subnegotiation(ip->telnet,
            TELNET_TELOPT_LINEMODE, (const char *)sb_dont, sizeof(sb_dont));
        break;
      }
      default:
        break;
...
static inline void on_telnet_environ(const struct telnet_environ_t * values,
    unsigned long size, interactive_t *ip) {
...
void set_linemode(interactive_t *ip) {
  ...
  const unsigned char sb_mode[] = {
      LM_MODE, MODE_EDIT | MODE_TRAPSIG
  };
  telnet_subnegotiation(
      ip->telnet, TELNET_TELOPT_LINEMODE,
      (const char *)sb_mode, sizeof(sb_mode));
...
void set_echo(interactive_t *ip, bool enable) {
  telnet_negotiate(
      ip->telnet, enable ? TELNET_WILL : TELNET_WONT,
          TELNET_TELOPT_ECHO);
  flush_message(ip);
}
