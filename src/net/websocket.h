...
// New handshakes present the on-disk chain; existing sessions keep the
// handshake they already completed. Returns 0 on success. 1 = port is
// not TLS websocket. 2 = cert/key would not load (same probe as the
// telnet path) or the vhost CTX is missing.
