...
int yylex(union YYSTYPE *yylval_param, struct YYLTYPE *yylloc_param, void *yyscanner);  // generated by Flex (lexer.autogen.cc)
void lpc_lex_newline(void *yyscanner);  // called from lexer.l's "\n" rule
void lexerror(const char *s);  // called from lexer_utils.cc helpers and lexer.l's native rules
// Graceful replacement for flex's stock exit(2) fatal handler; wired in
// via lexer.l's YY_FATAL_ERROR override. Defined in compiler.cc (needs
// error()).
[[noreturn]] void lpc_lex_fatal(const char *msg);
int lpc_lex_resolve_identifier(union YYSTYPE *yylval_param, struct YYLTYPE *yylloc_param, void *yyscanner);  // called from lexer.l's identifier rule
...
int parseMainEof(union YYSTYPE *yylval_param, void *yyscanner);  // called from lexer.l's <<EOF>> rule
int lpc_lex_badlex(unsigned char c, void *yyscanner);  // called from lexer.l's "." catch-all rule
...
// comes from the splice, its arguments from the file). Defined in lexer.l's
...
// lexer.l's trailer (touches the generated buffer API).
...
// Clear the active-scanner pointer when destroying its scanner (must be
// called right before yylex_destroy by every owner).
...
// scanning the base buffer. lexer.l's directive rule uses it to decide how
// the directive's terminating newline is consumed. Defined in
// lexer_utils.cc.
int lpc_lex_top_buffer_kind(void);

// Pushed-buffer bookkeeping used by the lexer.l-trailer helpers above
...
// zero policy, defined in lexer.l's trailer because the generated scanner's
...
// buffer is exhausted. Defined in lexer.l's trailer (BEGIN/YY_START are
