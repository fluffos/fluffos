...
void rule_parse_command(parse_node_t** result, parse_node_t* expr1, parse_node_t* expr2,
                        parse_node_t* expr3, parse_node_t* lvalue_list) {
  int p = lvalue_list->v.number;
  CREATE_LVALUE_EFUN(*result, TYPE_NUMBER, lvalue_list);
...
void rule_string(parse_node_t** result, const ScratchString* str) { CREATE_STRING(*result, str); }
...
void rule_template_literal(parse_node_t** result, const ScratchString* head, parse_node_t* expr,
                           parse_node_t* rest) {
...
void rule_template_parts_middle(parse_node_t** result, const ScratchString* mid, parse_node_t* expr,
                                parse_node_t* rest) {
...
parse_node_t* rule_opt_class_init_empty() { return 0; }
...
LPC_INT rule_functional_open(LPC_INT val) { return (val << 8) | FP_EFUN; }
...
#define LV_INDEX 4
  ...
  if (res->kind == NODE_BINARY_OP && res->v.number == F_TYPE_CHECK) res = res->l.expr;
  switch (res->kind) {
    default:
            ...
            case NODE_BINARY_OP:
              ...
              if ((node->v.number >= F_LOCAL && node->v.number <= F_MEMBER) ||
                ...
                if (node->r.expr->kind == NODE_BINARY_OP || node->r.expr->kind == NODE_TERNARY_OP) {
                  if (node->r.expr->v.number >= F_NN_RANGE_LVALUE &&
                      node->r.expr->v.number <= F_NR_RANGE_LVALUE)
                    yyerror(
...
void rule_expr_ref(parse_node_t** result, parse_node_t* lval) {
  ...
  if (!(context & ARG_LIST)) {
    ...
    switch (lval->kind) {
      ...
      case NODE_BINARY_OP:
        op = lval->v.number;
        if (op > F_RINDEX_LVALUE) yyerror("Illegal to make reference to range");
        break;
      default:
        op = 0;
...
void rule_expr_ternary(parse_node_t** result, parse_node_t* cond, parse_node_t* val1,
                       parse_node_t* val2) {
...
void rule_primary_expr_cast(parse_node_t** result, LPC_INT type, parse_node_t* expr) {
  *result = expr;
  ...
  if (exact_types && expr->type != type && expr->type != TYPE_ANY && expr->type != TYPE_UNKNOWN &&
      type != TYPE_VOID) {
    char buf[256];
    char* end = EndOf(buf);
    char* p;
...
void rule_primary_expr_defined_name(parse_node_t** result, ident_hash_elem_t* ihe) {
  ...
  if ((i = ihe->dn.local_num) != -1) {
    ...
    if (current_function_context) current_function_context->num_locals++;
...
function_context_t* rule_dollar_open() {
  ...
  if (current_function_context) current_function_context = current_function_context->parent;
  return saved;
}

void rule_primary_expr_dollar_expr(parse_node_t** result, function_context_t* saved_context,
                                   parse_node_t* expr) {
  parse_node_t* node;
...
void rule_primary_expr_member_arrow(parse_node_t** result, parse_node_t* expr,
  ...
  if (expr->type == TYPE_ANY) {
    ...
    (*result)->l.number =
        lookup_class_member(CLASS_IDX(expr->type), identifier, &((*result)->type));
  }
}

void rule_primary_expr_member_dot(parse_node_t** result, parse_node_t* expr,
                                  const ScratchString* identifier) {
  if (expr->type == TYPE_ANY) {
    int cmi;
    unsigned short tp;
...
void rule_primary_expr_member_optional(parse_node_t** result, parse_node_t* expr,
                                       const ScratchString* identifier) {
...
void rule_primary_expr_index_optional(parse_node_t** result, parse_node_t* expr,
                                      parse_node_t* idx) {
  CREATE_BINARY_OP(*result, F_MAP_INDEX_OPTIONAL, 0, idx, expr);
  (*result)->type = TYPE_ANY;
}

void rule_primary_expr_range_nn(parse_node_t** result, parse_node_t* expr, parse_node_t* expr1,
                                parse_node_t* expr2) {
  if (!CONFIG_INT(__RC_OLD_RANGE_BEHAVIOR__)) {
    if (CONFIG_INT(__RC_WARN_OLD_RANGE_BEHAVIOR__)) {
      if (expr->type != TYPE_MAPPING && expr2->kind == NODE_NUMBER && expr2->v.number < 0)
        yywarn(
            "A negative constant as the second element of arr[x..y] no longer means indexing from "
            "the end.  Use arr[x..<y]");
    }
  }
...
void rule_primary_expr_range_rn(parse_node_t** result, parse_node_t* expr, parse_node_t* expr1,
                                parse_node_t* expr2) {
  *result = make_range_node(F_RN_RANGE, expr, expr1, expr2);
}

void rule_primary_expr_range_rr(parse_node_t** result, parse_node_t* expr, parse_node_t* expr1,
                                parse_node_t* expr2) {
...
void rule_primary_expr_range_nr(parse_node_t** result, parse_node_t* expr, parse_node_t* expr1,
                                parse_node_t* expr2) {
  if (expr2->kind == NODE_NUMBER && expr2->v.number <= 1)
  ...
  if (exact_types) {
    switch (expr->type) {
      ...
      case TYPE_BUFFER:
        ...
        if (!IS_TYPE(idx->type, TYPE_NUMBER)) type_error("Bad type of index", idx->type);
        break;
      default:
        if (expr->type & TYPE_MOD_ARRAY) {
          (*result)->type = expr->type & ~TYPE_MOD_ARRAY;
          if ((*result)->type != TYPE_ANY) *result = add_type_check(*result, (*result)->type);
          if (!IS_TYPE(idx->type, TYPE_NUMBER)) type_error("Bad type of index", idx->type);
        } else {
...
void rule_primary_expr_anon_func(parse_node_t** result, func_block_t* saved_block, argument_t* arg,
  ...
  *result = new_node();
  ...
  if (current_function_context->bindable) (*result)->v.number |= 0x10000;
...
void rule_primary_expr_functional_2(parse_node_t** result, LPC_INT val,
                                    parse_node_t* opt_arg_list) {
  *result = new_node();
  (*result)->kind = NODE_FUNCTION_CONSTRUCTOR;
  (*result)->type = TYPE_FUNCTION;
  (*result)->v.number = val;
          ...
          for (argn = 0; argn < num; argn++) {
            ...
            if (argp[i] == 0) {
              ...
              if (tmp == TYPE_NUMBER && argp[i] == TYPE_REAL) {
                ...
                if (!argp[i]) enode->v.expr = promote_to_float(enode->v.expr);
              }
              if (tmp == TYPE_REAL && argp[i] == TYPE_NUMBER) {
                for (i++; argp[i] && argp[i] != TYPE_REAL; i++);
                if (!argp[i]) enode->v.expr = promote_to_int(enode->v.expr);
              }
            }
            while (argp[i] != 0) i++;
            argp += i + 1;
...
void rule_primary_expr_functional_3(parse_node_t** result, parse_node_t* expr) {
  if (current_function_context->num_locals) yyerror("Illegal to use local variable in functional.");
...
void rule_function_call_efun(parse_node_t** result, LPC_INT efun_idx, parse_node_t* opt_arg_list,
                             LPC_INT saved_context, LPC_INT saved_refs) {
  context = saved_context;
...
void rule_function_call_new(parse_node_t** result, parse_node_t* opt_arg_list,
                            LPC_INT saved_context, LPC_INT saved_refs) {
  ident_hash_elem_t* ihe;
  int f;
...
void rule_function_call_new_class(parse_node_t** result, ident_hash_elem_t* ihe,
                                  parse_node_t* class_init) {
  parse_node_t* node;
...
void rule_function_call_new_class_undef(parse_node_t** result, const ScratchString* name,
                                        parse_node_t* class_init) {
...
void rule_function_call_defined_name(parse_node_t** result, ident_hash_elem_t* ihe,
  ...
  *result = opt_arg_list;
  if ((f = ihe->dn.function_num) != -1) {
    ...
    *result = validate_efun_call(f, opt_arg_list);
    ...
    if (current_function_context) current_function_context->num_locals++;
  } else if ((i = ihe->dn.global_num) != -1 && (VAR_TEMP(i)->type & ~DECL_MODS) == TYPE_FUNCTION) {
    parse_node_t* expr;
    parse_node_t* func_node;
...
void rule_function_call_indexed(parse_node_t** result, parse_node_t* expr, parse_node_t* idx,
                                parse_node_t* opt_arg_list, LPC_INT saved_context,
                                LPC_INT saved_refs) {
...
void rule_function_call_arrow(parse_node_t** result, parse_node_t* expr,
                              const ScratchString* identifier, parse_node_t* opt_arg_list,
                              LPC_INT saved_context, LPC_INT saved_refs) {
  ident_hash_elem_t* ihe;
  int f;
...
void rule_function_call_star(parse_node_t** result, parse_node_t* expr, parse_node_t* opt_arg_list,
                             LPC_INT saved_context, LPC_INT saved_refs) {
  parse_node_t* sub_expr;
...
void rule_comma_expr(struct parse_node_t** result, struct parse_node_t* expr1,
                     struct parse_node_t* expr2) {
  CREATE_TWO_VALUES(*result, expr2->type, pop_value(expr1), expr2);
}

void rule_expr_nullish(struct parse_node_t** result, struct parse_node_t* expr1,
                       struct parse_node_t* expr2) {
  CREATE_NULLISH(*result, expr1, expr2);
}

void rule_expr_lor(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  CREATE_LAND_LOR(*result, F_LOR, expr1, expr2);
  if (IS_NODE(expr1, NODE_LAND_LOR, F_LOR)) expr1->kind = NODE_BRANCH_LINK;
}

void rule_expr_land(struct parse_node_t** result, struct parse_node_t* expr1,
                    struct parse_node_t* expr2) {
  CREATE_LAND_LOR(*result, F_LAND, expr1, expr2);
  if (IS_NODE(expr1, NODE_LAND_LOR, F_LAND)) expr1->kind = NODE_BRANCH_LINK;
}

void rule_expr_or(struct parse_node_t** result, struct parse_node_t* expr1,
                  struct parse_node_t* expr2) {
  int t1 = expr1->type, t3 = expr2->type;
  if (is_boolean(expr1) && is_boolean(expr2)) yywarn("bitwise operation on boolean values.");
  if ((t1 & TYPE_MOD_ARRAY) || (t3 & TYPE_MOD_ARRAY)) {
    ...
    *result = binary_int_op(expr1, expr2, F_OR, "|");
}

void rule_expr_xor(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  *result = binary_int_op(expr1, expr2, F_XOR, "^");
}

void rule_expr_and(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  int t1 = expr1->type, t3 = expr2->type;
  if (is_boolean(expr1) && is_boolean(expr2)) yywarn("bitwise operation on boolean values.");
  if ((t1 & TYPE_MOD_ARRAY) || (t3 & TYPE_MOD_ARRAY)) {
    ...
    *result = binary_int_op(expr1, expr2, F_AND, "&");
}
...
void rule_expr_eq(struct parse_node_t** result, struct parse_node_t* expr1,
                  struct parse_node_t* expr2) {
...
void rule_expr_ne(struct parse_node_t** result, struct parse_node_t* expr1,
                  struct parse_node_t* expr2) {
  if (exact_types && !compatible_types2(expr1->type, expr2->type)) {
    char buf[256];
    char* end = EndOf(buf);
...
void rule_expr_order(struct parse_node_t** result, struct parse_node_t* expr1, LPC_INT op,
                     struct parse_node_t* expr2) {
  if (exact_types) {
    int t1 = expr1->type;
    int t3 = expr2->type;
...
void rule_expr_lt(struct parse_node_t** result, struct parse_node_t* expr1,
                  struct parse_node_t* expr2) {
  if (exact_types) {
    int t1 = expr1->type, t3 = expr2->type;
...
void rule_expr_add(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  int result_type;
  ...
  switch (expr1->kind) {
    case NODE_NUMBER:
      if (expr1->v.number == 0 && (expr2->type == TYPE_NUMBER || expr2->type == TYPE_REAL)) {
        *result = expr2;
        break;
      }
...
void rule_expr_sub(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  int result_type;
  ...
  if (exact_types) {
    ...
    } else if ((t1 == TYPE_REAL && t3 == TYPE_NUMBER) || (t3 == TYPE_REAL && t1 == TYPE_NUMBER)) {
      result_type = TYPE_REAL;
...
void rule_expr_mul(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  int result_type;
  ...
  if (exact_types) {
    ...
    if (t1 == t3) {
      ...
      switch (t) {
        ...
        default:
          type_error(
              (t1 == TYPE_ANY) ? "Bad argument number 2 to '*'" : "Bad argument number 1 to '*'",
              t);
          result_type = TYPE_ANY;
      }
    } else if ((t1 == TYPE_NUMBER && t3 == TYPE_REAL) || (t1 == TYPE_REAL && t3 == TYPE_NUMBER)) {
      result_type = TYPE_REAL;
    } else {
      char buf[256];
      char* end = EndOf(buf);
...
void rule_expr_mod(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  *result = binary_int_op(expr1, expr2, F_MOD, "%");
}

void rule_expr_div(struct parse_node_t** result, struct parse_node_t* expr1,
                   struct parse_node_t* expr2) {
  int result_type;
  ...
  if (exact_types) {
    ...
    if (t1 == t3) {
      ...
      if (t == TYPE_REAL || t == TYPE_NUMBER)
        ...
        type_error(t1 == TYPE_ANY ? "Bad argument 2 to '/'" : "Bad argument 1 to '/'", t);
        result_type = TYPE_ANY;
      }
    } else if ((t1 == TYPE_NUMBER && t3 == TYPE_REAL) || (t1 == TYPE_REAL && t3 == TYPE_NUMBER)) {
      result_type = TYPE_REAL;
...
void rule_expr_compl(struct parse_node_t** result, struct parse_node_t* expr) {
  if (exact_types && !IS_TYPE(expr->type, TYPE_NUMBER)) type_error("Bad argument to ~", expr->type);
  if (expr->kind == NODE_NUMBER) {
    *result = expr;
    (*result)->v.number = ~(*result)->v.number;
  } else {
...
void rule_opt_arg_list_empty(struct parse_node_t** result) { CREATE_EXPR_LIST(*result, 0); }
...
void rule_arg_list_multi(struct parse_node_t** result, struct parse_node_t* list,
                         struct parse_node_t* expr) {
  expr->kind = 0;
  *result = list;
  (*result)->kind++;
...
void rule_opt_pair_list_empty(struct parse_node_t** result) { CREATE_EXPR_LIST(*result, 0); }
...
void rule_pair_list_multi(struct parse_node_t** result, struct parse_node_t* list,
                          struct parse_node_t* expr) {
...
void rule_assoc_pair(struct parse_node_t** result, struct parse_node_t* expr1,
                     struct parse_node_t* expr2) {
