...
graph TB
    ...
    subgraph "Token layer"
        ...
        LEX -->|"# directive lines, macro table, #if stack"| LRPP("lexer_rules_pp.cc (pp state in g_compile)")
...
- **There is no byte-stream or token-stream abstraction — the scanner IS the interface.** `compile_file()` (and `stage_output.cc`) create a reentrant Flex scanner directly (`yylex_init_extra`, destroyed by a scope guard declared BEFORE the cleanup guard, so unwind teardown never touches a destroyed scanner), point it at source with `start_new_file(view, scanner)` or `start_new_file_fd(fd, ...)` (zero copy: `read(2)` lands the bytes in the arena block flex scans in place via `yy_scan_buffer`), and pull tokens with plain `yylex()`. `#include` contents and macro splices use the same in-place mechanism, so every input in the compiler is one kind of thing: an in-memory Flex buffer over arena memory. The historical `LexStream` hierarchy, the `LexTokenStream` wrapper, and the `YY_INPUT` refill bridge are all gone. Whoever destroys a scanner calls `lpc_lex_scanner_destroyed()` first (clears the active-scanner global; an aborted compile skips `end_new_file`, and stale reads were a real use-after-free pinned by `CompileEntry.FatalAbortThenRecompile`). `start_new_file(..., keep_macros=true)` retains the user `#define` table across REPL chunks (the `#if` stack always resets).
...
- **`base/internal/scratchpad.cc`, `base/internal/scratchpad.h`** (moved out of `compiler/internal/`: ownership no longer sits with the compiler): the compile-lifetime **monotonic bump arena**. Allocation is a pointer bump, individual deallocation is a no-op, and `scratch_destroy()`/`ScratchArena::reset()` bulk-frees everything (which is what makes every `error()` unwind path leak-free with zero per-allocation ownership plumbing) — but a compile does not decide when that happens. `compile_file()` BORROWS a `ScratchArena` from its caller and never resets or frees it; a caller that doesn't supply one gets the shared default arena, which recycles on the way IN to the next compile rather than on the way out. That is what lets `Diagnostic` records live on the arena and still be readable once the compile has returned (lpcshell renders them after the fact) — the caller decides when they die, by resetting or dropping the arena, and MUST call `compiler_drop_arena_state()` immediately before doing so (see its declaration in `compiler.h`). Build every transient compile string as a **`ScratchString`** (`std::basic_string` over `ScratchAllocator`; `ScratchVector<T>` likewise) and materialize a parser token with `scratch_new_string()` — the Bison `%union`'s `string` member is a `ScratchString *` whose object itself lives in the arena, and shared strings on the value stack use the union's separate `shared_string` member so the two lifetimes are type-distinguished (see the `function` production). Anything that outlives the arena it was allocated from (macro table, predefines, program data) must NOT live on the arena — copy out at the boundary. Arena-backed objects stored in memory that survives the compile (the scanner context's accumulators) are re-initialized per compile in `lpc_lex_reset_context()`.

---

## Grammar as a machine contract (`grammar.ebnf`, `lpc-grammar.json`)

The grammar-contract assets all live in `tools/lpc-syntax/`:
`grammar.ebnf` is composed of three layers by
`tools/lpc-syntax/generate_ebnf.py` (CMake target `generate_ebnf`): a
hand-authored **Lexical** layer and **Preprocessor** layer
(`grammar_lexical.ebnf.in` — terminals, escapes, templates, text
blocks, directive grammar; regeneration never loses them) plus the
**Syntax** layer generated from this directory's `grammar.y` via
`bison --xml`. The same run emits `lpc-grammar.json` —
keywords/operators/punctuation/directives/productions — which drives
the dependency-free JS tokenizer, syntax highlighter, formatter, and
the VS Code extension in `tools/lpc-syntax/`. The generator asserts
every `grammar.y` terminal is categorized, so the artifacts cannot
silently go stale.
