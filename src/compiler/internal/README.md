...
### 1. Token stream layer (`LexStream.h`, `lexer_utils.cc`)
- **There is no byte-stream abstraction.** `LexTokenStream::load()` takes the complete source TEXT (`std::string_view`), installed as a native in-memory Flex base buffer via `lpc_lex_set_source()` (`yy_scan_bytes` — which copies, so the caller's text may be transient). This is exactly the mechanism `#include` contents and macro splices already use, so every input in the compiler is one kind of thing: an in-memory Flex buffer. Callers with a file simply slurp it (`load_object` in simulate.cc); the historical `LexStream`/`FileLexStream`/`StringLexStream` `read()` hierarchy and the `YY_INPUT` refill bridge are gone (`YY_INPUT` is a constant end-of-input for the one defensive placeholder-buffer path).
- **`LexTokenStream`**: owns a reentrant Flex scanner via RAII (`yylex_init_extra`/`yylex_destroy`) and exposes `load(source, session)` + `next(&yylval)` + `scanner()`. This is the consolidation point `compile_file()` uses instead of wiring up scanner creation and the token-pull loop by hand. A single instance can be `load()`ed more than once, reusing the same scanner rather than tearing one down and building another — the mechanism a REPL needs to feed many small statements through one session cheaply. `session` is a `LexerSession` (see Module 2); passing the same session across `load()` calls keeps `#define` state alive between chunks.
- **`lexer_utils.cc`**: `start_new_file()` (installs the slurped source as the base buffer, guaranteeing a trailing newline), the pushed-buffer bookkeeping (the kind stack behind `lpc_lex_push_string_buffer()`/`lpc_lex_pop_pushed_buffer()`: PLAIN pushbacks, EXPANSION splices with their provenance frames, INCLUDE contents), the `#include` metadata stack (`IncState{line,file,file_id}`: `lpc_lex_handle_include()` slurps the file and pushes its content as a Flex buffer; the pop restores file identity and closes the `save_file_info` line accounting), macro expansion at identifier-resolution time (`lpc_lex_resolve_identifier()`), predefine/include-path management, and `LexTokenStream`'s/`start_new_file()`'s implementations. Known scope boundary: this lexer state is still file-scope static, not per-`LexTokenStream`-instance — two instances alive at once would corrupt each other's state. Not a problem today: `compile_file()`'s reentrancy guard already prevents concurrent compiles.
