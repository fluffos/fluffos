 ...
 * the code that reads raw characters instead of going through the DFA.
 *
 * Three places do that -- the function-like-macro argument collector
 * (lexer_utils.cc), the directive-line completer and the two directive
 * text helpers (lexer_rules_pp.cc) -- and each used to carry its own
 * hand-rolled version. They disagreed, and the disagreements were bugs:
 * an apostrophe in a comment inside an argument list opened a character
 * literal that ran to end of file (#1362), and an apostrophe anywhere on
 * a directive line stopped the line completer from seeing a block comment
 * that spanned lines, so the comment's continuation was compiled as code.
 * Both are the same mistake -- treating a lone "'" as a quote that scans
 * forward for a partner -- made independently in two places.
 *
 ...
 * it: every raw reader consumes input through this, so none of them can
 * drift from the DFA or from each other again.
 */
...
struct UnitInfo {
  ...
  int newlines = 0;
  // A "/*" seen INSIDE a block comment -- lexer.l's SC_BLOCK_COMMENT warns
  // about it, so a caller that replaces that scan warns the same way.
  bool nested_comment_open = false;
...
//
// Split out because the directive-line completer is handed control while a
// comment opened on the directive line is still open: it finishes THAT
// comment through this, then classifies the rest of the logical line with
// scan_one_unit(), so both halves obey one set of rules.
template <class Src>
...
UnitInfo scan_one_unit(Src& src, int depth) {
  ...
  if (c == '/') {
    ...
    if (c2 == '*') {
      src.advance();
      src.advance();
      return finish_block_comment(src);
    }
    if (c2 == '/') {
      src.advance();
      src.advance();
      return finish_line_comment(src);
    }
