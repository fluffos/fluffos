...
#ifdef LPC_TO_C
#ifdef F_CRYPT
void c_crypt P3(svalue *, ret, svalue *, s0, svalue *, s1)
{
    char *res, salt[2];
    char *choice =
    "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ./";

    if (s1->type == T_STRING && SVALUE_STRLEN(s1) >= 2) {
	salt[0] = s1->u.string[0];
	salt[1] = s1->u.string[1];
    } else {
	salt[0] = choice[random_number(strlen(choice))];
	salt[1] = choice[random_number(strlen(choice))];
    }
#if defined(sun) && !defined(SunOS_5)
    res = string_copy(_crypt(s0->u.string, salt));
#else
    res = string_copy(crypt(s0->u.string, salt));
#endif
...
#ifdef F_LOCALTIME
void c_localtime P2(svalue *, ret, svalue *, s0)
{
    struct tm *tm;
    struct vector *vec;
    time_t lt;
...
void c_rusage P1(svalue *, ret)
{
    ...
    i = timer(clock);		/* 0 == success, -1 == failure */
    m = allocate_mapping(2);
...
*/
void c_malloc_check P1(svalue *, ret)
{
    C_NUMBER(ret, NXMallocCheck());
}
#endif
...
/* NeXT specific efun for setting the debugging level of NeXT's built-in
   malloc.
*/
void c_malloc_debug P2(svalue *, ret, svalue *, s0)
{
    int level;
	...
	C_NUMBER(ret, malloc_debug(level));
    }
}
#endif
