...
#define NUM_FILES 5
  static const char *outfiles[NUM_FILES] = {EFUN_TABLE, OPC_PROF, OPCODES, EFUN_PROTO, EFUN_DEFS};
...
static void handle_build_func_spec(char *command) {
  ...
  for (i = 0; i < num_packages; i++) {
    sprintf(buf, "%s -I. packages/%s.spec >>%s", command, packages[i], FUNC_SPEC_CPP);
    system(buf);
  }
