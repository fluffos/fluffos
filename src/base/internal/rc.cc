...
const FlagEntry INT_FLAGS[] = {
    ...
    {"async drain eval budget", __RC_ASYNC_DRAIN_EVAL_BUDGET__, 0, 0, INT_MAX,
     "Timing & Lifecycle",
     "How long one turn of the promise microtask drain may hold the event loop, in the same microseconds `maximum evaluation cost` is measured in (FluffOS meters eval cost with a wall-clock timer, so the two units are the same thing). When the turn has spent this much, the remaining deliveries are deferred past the next I/O poll -- the loop reads sockets, schedules commands and fires timers, then the drain resumes. Deferral is not an error and nothing is dropped. This replaced a count of deliveries, which could not bound anything: each delivery is armed with a fresh `maximum evaluation cost`, so a batch of N had a worst case of N times that -- measured at 17-21 SECOND freezes with the old default of 512. A budget bounds the turn at itself plus at most one delivery's eval cost, which is the same worst case any ordinary command or call_out already has. 0 (the default) uses a built-in budget of one millisecond."},
