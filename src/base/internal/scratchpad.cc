...
// any compile.
//
// Deliberately leaked (`new`, never deleted) rather than a plain function-
// local static: a plain static's destructor runs at exit() via the normal
// static-storage-duration teardown, in REVERSE order of construction. This
// one is constructed lazily, on the driver's first compile -- necessarily
// AFTER g_compile (compiler.cc's namespace-scope CompileState, constructed
// before main() runs) -- so (confirmed by tracing both destructors) it IS
// destroyed before g_compile at exit. If compiler_diags (part of g_compile,
// destroyed after) still holds a Diagnostic whose ScratchVector member
// (notes, expansions, fixits, ranges, included_from) lives on a chunk this
// destructor frees, destroying compiler_diags afterward reads freed memory.
//
// In practice this needs a large compile at some point in the driver's
// life to bite: chunk 0 is almost always the persistent STATIC block
// (ensure_init borrows base_storage for the first arena that needs one, and
// 1MB covers virtually every compile -- see kBaseSize), and freeing that
// chunk is a deliberate no-op (~ScratchArena skips i==0 when base_is_static),
// so a Diagnostic living there survives this destructor untouched. The
// hazard is live only once some compile has pushed this arena's retained
// chunk count past 1 (a genuinely heap chunk survives to be freed here) or
// spilled into an overflow chunk -- but nothing in the contract prevents
// that, and there are several exit() call sites downstream of a compile
// (shutdownMudOS(), vm_start()'s boot-failure catch block, master.cc's
// uid/backbone checks, ...), some of them one compile failure away from
// running with no later compile to have cleared compiler_diags first.
// Leaking sidesteps the ordering question entirely: nothing here is freed
// until the process exits and the OS reclaims everything anyway.
ScratchArena& scratch_default_arena() {
  static ScratchArena& a = *new ScratchArena();
  return a;
}
