...
void f_children(void) {
  ...
  vec = ObjectTable::children(sp->u.string);
...
void f_mud_status(void) {
  ...
  if (verbose) {
    ...
    outbuf_add(&ob, "\n");
#endif
    tot = ObjectTable::show_otable_status(&ob, verbose);
    outbuf_add(&ob, "\n");
    ...
    tot = ObjectTable::show_otable_status(&ob, verbose) + heart_beat_status(&ob, verbose) +
...
void f_memory_info(void) {
  ...
  if (st_num_arg == 0) {
    ...
    tot = total_prog_block_size + total_array_size + total_class_size + total_mapping_size +
          ...
          users_num(1) * sizeof(interactive_t) + ObjectTable::show_otable_status(0, -1) +
