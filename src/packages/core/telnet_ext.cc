...
void f_send_gmcp() {
  auto ip = current_object->interactive;
  if (ip && ip->telnet) {
    telnet_subnegotiation(ip->telnet, TELNET_TELOPT_GMCP, sp->u.string->c_str(), sp->u.string->size());
    flush_message(ip);
  }
  pop_stack();
}
...
void on_telnet_do_zmp(const char** argv, unsigned long argc, interactive_t* ip) {
  ...
  for (int i = 1; i < argc; i++) {
    arr->item[i].u.string = std::string {argv[i]};
...
void f_send_zmp() {
  ...
  if (ip && ip->telnet) {
    telnet_begin_zmp(ip->telnet, (sp - 1)->u.string->c_str());

    for (int i = 0; i < sp->u.arr->size; i++) {
      if (sp->u.arr->item[i].type == T_STRING) {
        telnet_zmp_arg(ip->telnet, sp->u.arr->item[i].u.string->c_str());
      }
    }
