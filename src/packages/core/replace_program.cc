...
// findstring() arrives via package_api.h -> base/std.h; AGENTS.md section 12
// forbids including base/internal headers directly.
...
static bool replace_program_is_noop(program_t* cur_prog, program_t* new_prog) {
  ...
  // The "#global_init#" skip below is load-bearing for BOUNDS safety, not
  // just for semantics: handle_functions() (compiler.cc) shortens
  // comp_last_inherited by one when the last inherit's final defined
  // function is APPLY___INIT (== "#global_init#"), so that entry's
  // runtime_index is exactly the top program's last_inherited -- one past
  // the end of a function_flags[] sized last_inherited + num_func,
  // whenever the top program defines no functions of its own
  // (`inherit "b";` and nothing else). function_flags points into the
  // same DMALLOC block as the rest of program_t's arrays, so indexing it
  // there would be a silent intra-block over-read that no sanitizer
  // flags.
  //
  // The `!= nullptr` guards below are therefore NOT a hole: a null key
  // means the name is not in the shared-string table at all, and every
  // live program holds a ref on each of its own function names
  // (make_shared_string() in define_new_function(), released by
  // deallocate_program()). So a null key proves no live program has a
  // function by that name, which in turn proves cur_prog's resolved table
  // cannot contain that key -- the skip is vacuous exactly when it is
  // unavailable, and the out-of-range entry cannot exist. Failing closed
  // here instead would needlessly disable the fast path outright on a
  // mudlib where no loaded program has initialized globals.
