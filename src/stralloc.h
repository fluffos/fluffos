...
#ifdef NOISY_STRING
#define NDBG(x)                             \
  if (strcmp(STRING(x), NOISY_STRING) == 0) \
  debug_message("%s - %d\n", STRING(x), REFS(x)), bp()
#else
...
#define MSTR_UPDATE_SIZE(x, y)            \
  SAFE(ADD_STRING_SIZE(y - MSTR_SIZE(x)); \
       MSTR_BLOCK(x)->size = (y > UINT_MAX ? UINT_MAX : y);)

#define FREE_MSTR(x)                                                    \
  SAFE(DEBUG_CHECK(MSTR_REF(x) != 1,                                    \
                   "FREE_MSTR used on a multiply referenced string\n"); \
       svalue_strlen_size = MSTR_SIZE(x);                               \
       SUB_NEW_STRING(svalue_strlen_size, sizeof(malloc_block_t));      \
       FREE(MSTR_BLOCK(x)); SUB_STRING(svalue_strlen_size);)
...
#define COUNTED_STRLEN(x)                              \
  ((svalue_strlen_size = MSTR_SIZE(x)),                \
   svalue_strlen_size != UINT_MAX ? svalue_strlen_size \
                                  : strlen((x) + UINT_MAX) + UINT_MAX)
...
#define SVALUE_STRLEN(x)                                          \
  (((x)->subtype &STRING_COUNTED) ? COUNTED_STRLEN((x)->u.string) \
                                  : strlen((x)->u.string))
